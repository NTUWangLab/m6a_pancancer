...
my_data = read.csv('~/diff/data/ESTIMATE.csv',header=T,check.names=F)

other_data = read.csv("~/projects/Tcell-pancancer/cluster/cluster.csv",,header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  pdf(paste(file_name,"/",name,".pdf",sep=''),width=length(unique(dat[,1]))+1,height = 8)
  ...
  p <- ggboxplot(dat, x = "Gene", y = "value",
                 color = "Group", palette = c('#ff5126','#d3f6d1','#ea5455','#323643'),
...
######
pdf(paste(file_name,"/fig1.pdf",sep=''),width = length(colnames(exp_file))*8+3,height = 8*length(out_put_list)/8)

eval(parse(text = paste('print(cowplot::plot_grid(',paste(out_put_list, collapse = ","),', ncol=8,labels=out_put_list,label_size=20))',sep='')))
