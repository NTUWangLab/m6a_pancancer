...
for (tumor_id in tumor_list)
{
  ...
  # Read drug data for the current tumor type
  ...
  # Extract relevant data for the current tumor type
  ...
  # Read m6a data for the current tumor type
  ...
  # Compute correlation between gene expression and m6a data
  ...
  p = ggplot(temp_data, aes(x = Col, y = Row, color = rvalue, size = abs(-log10(pvalue)))) +
    geom_point() +
    scale_color_gradient2(low = "green3", high = "red3") +
    scale_size_continuous(range = c(1, 10)) +
    labs(title = tumor_id, x = "", y = "", color = "R", size = "-log10(P)") +
    ggtitle(tumor_id)
...
# Combine all tumor type plots into a single PDF
