...
for(name in tumor_list)
{
  ...
  p <- ggboxplot(dat, x = "Gene", y = "value",
                 fill = "Group", palette = c('#f9c535', 'purple3','#6ce6dd','#334854', '#6acafc', '#8e9fe6'),
...
######
pdf(paste(file_name,"/fig1.pdf",sep=''),width = length(colnames(exp_file))*3.5,height = 8*length(out_put_list)/4)
