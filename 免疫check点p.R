...
data_list = c('Inhibitory','Stimulaotry')
for(names in data_list)
{
df = read.csv(paste('immune_check/',names,'/risk/p.csv',sep=''),header=T,row.names=1)
...
#immune_reg
myColor <- colorRampPalette(c( '#29c6cd', "white", '#ffd00c'))(paletteLength)
...
pdf(paste('immune_check/',names,'/risk/pheatmap.pdf',sep=''),length(colnames(df))/2+2,length(rownames(df))/4+3)
