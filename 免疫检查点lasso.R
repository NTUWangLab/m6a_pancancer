...
for(classify in data_list)
{
  ...
  out_put_list <- c()
  file_name = paste0('immune_check/',classify,'/risk')
  ...
  my_data = read.csv(paste0('~/diff/data/immune_check/',classify,'.csv'),header=T,check.names=F)
  ...
  for(name in tumor_list)
  {
    ...
    # p <- ggboxplot(dat, x = "Gene", y = "value",
    #                color = "Group", palette = c('#d01257','#29c6cd','#f33535','#34495e'),
    #                add = c("mean"),x.text.angle=60)
    # 
    #中间凹陷
    p <- ggboxplot(dat, x = "Gene", y = "value",
                   fill = "Group", palette  = c('#ffd00c','#29c6cd','#febe7e','#34495e'),
                   x.text.angle=60)
