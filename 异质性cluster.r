...
my_data = read.csv('~/diff/data/yzx.csv',header=T,check.names=F)
...
for(name in tumor_list)
{
  ...
  p <- ggboxplot(dat, x = "Gene", y = "value",
                color = "Group", palette = c('red3','yellow2','#9fbdb0','#ffc93c'),
                x.text.angle=60)
  #中间凹陷
  # p <- ggboxplot(dat, x = "Gene", y = "value",
...
######
pdf(paste(file_name,"/fig1.pdf",sep=''),width = length(colnames(exp_file))*4+3,height = 8*length(out_put_list)/4)
